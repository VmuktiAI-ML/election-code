...
# from sort import Sort # sort.py needs to be available
...
import threading # Keep for api_upload_thread
import torch # Added for explicit CUDA checks and because YOLO uses it.
...
# YOLO model will be initialized per-worker process to avoid CUDA multiprocessing issues.
# Global model initialization is removed.
...
def get_output_rtmp(input_url):
    ...
    return f'rtmp://vivek.vmukti.com:80/live-record/{last_part}-AI'
...
def upload_frame_to_azure(image_bytes, camera_id, container_name, blob_folder, conn_str):
    ...
    try:
        ...
        timestamp_str = (datetime.now() + timedelta(hours=5, minutes=30)).strftime('%Y%m%d%H%M%S%f')
...
def handle_api_and_upload(frame_to_upload, camera_id, current_total_entry_count_for_payload, alert_url,
    ...
    api_payload = {
        "cameradid": camera_id,
        "sendtime": (datetime.now() + timedelta(hours=5, minutes=30)).strftime('%Y-%m-%d %H:%M:%S.%f')[:-3],
        "imgurl": img_url_azure if img_url_azure else "",
        "an_id": analytics_id_val,
        "totalcount": current_total_entry_count_for_payload,
        "ImgCount": 4
    }
...
def camera_worker(rtmp_url, line_points):
    # Import Sort here if it's not needed globally or to ensure it's fresh for the process
    # However, if Sort has no problematic global state, importing at top level is fine.
    # Assuming Sort is okay as a top-level import.
    from sort import Sort # Make sure sort.py is in PYTHONPATH or same directory

    camera_id = rtmp_url.split('/')[-1]
    print(f"[{camera_id}] Worker started for {rtmp_url} with line: {line_points}")

    # --- Model Initialization (Moved to worker process) ---
    worker_yolo_model = None
    try:
        print(f"[{camera_id}] Initializing YOLO model ({MODEL_PATH}) for this worker...")
        # This call to YOLO() is where PyTorch might try to initialize CUDA.
        worker_yolo_model = YOLO(MODEL_PATH)

        if torch.cuda.is_available(): # This call can trigger the CUDA warning
            print(f"[{camera_id}] PyTorch reports CUDA is available. YOLO model may use GPU.")
            # You could add worker_yolo_model.to('cuda') if you want to be explicit and YOLO doesn't do it.
        else:
            print(f"[{camera_id}] PyTorch reports CUDA is NOT available. YOLO model will use CPU.")
    except Exception as e:
        print(f"[{camera_id}] CRITICAL: Failed to initialize YOLO model in worker. Error: {e}. This worker will terminate and attempt restart.")
        import traceback
        traceback.print_exc()
        # Raise an error that the main exception handler in this worker will catch, leading to a restart attempt.
        raise RuntimeError(f"YOLO Model initialization failed for {camera_id}") from e
    ...
    while True:
        ...
        try:
            ...
            # OpenCV's VideoCapture will use environment variables for FFmpeg options (like timeout)
            cap = cv2.VideoCapture(rtmp_url)
            if not cap.isOpened():
                print(f"[{camera_id}] ❌ Could not open stream: {rtmp_url}. Retrying in 1s...")
                time.sleep(1)
                continue
            ...
            while True:
                success, frame = cap.read() # This might use OPENCV_FFMPEG_READ_ATTEMPTS
                if not success:
                ...
                # Use the worker-specific model instance
                results = worker_yolo_model(processed_frame, stream=False, verbose=False,
                ...
                for res in tracked_objects:
                    ...
                    if obj_id in prev_region and current_region[obj_id] and prev_region[obj_id]:
                        if prev_region[obj_id] == 'A' and current_region[obj_id] == 'B':
                            if obj_id not in current_session_entry_ids:
                ...
                if new_entry_detected_this_frame:
                ...
                if resumable_entry_count > 0 and \
                   ...
                   resumable_entry_count > last_api_sent_for_milestone:
                    ...
                    api_upload_thread = threading.Thread(
                        ...
                        args=(
                            frame_copy_for_upload, camera_id, current_milestone_to_send,
            ...
            if pipe and pipe.poll() is None:
                ...
                pipe.terminate(); pipe.wait()
            print(f"[{camera_id}] Restarting worker after unexpected error in 1s...") # Reduced from 5s
            current_session_entry_ids.clear()
            time.sleep(1)
...
def run_all_cameras():
    ...
    for camera_info in cameras_to_process:
        process = multiprocessing.Process(target=camera_worker, args=(camera_info['url'], camera_info['line']), daemon=True)
    ...
    try:
        while True:
            if not any(p.is_alive() for p in processes) and processes:
                print("All camera processes have finished or died. Exiting main program.")
                break
            time.sleep(10)
    except KeyboardInterrupt: print("\n🔴 KeyboardInterrupt received. Exiting main program...")

if __name__ == "__main__":
    # --- Environment Variable Settings for OpenCV/FFmpeg ---
    # To potentially mitigate "grabFrame packet read max attempts exceeded"
    os.environ['OPENCV_FFMPEG_READ_ATTEMPTS'] = '8192' # Default is 4096
    # To potentially reduce blocking time on unresponsive streams (10 seconds timeout in microseconds)
    # This sets the -timeout FFmpeg option for the input context.
    os.environ['OPENCV_FFMPEG_CAPTURE_OPTIONS'] = 'timeout;10000000'
    print(f"INFO: Set OPENCV_FFMPEG_READ_ATTEMPTS to {os.environ.get('OPENCV_FFMPEG_READ_ATTEMPTS')}")
    print(f"INFO: Set OPENCV_FFMPEG_CAPTURE_OPTIONS to {os.environ.get('OPENCV_FFMPEG_CAPTURE_OPTIONS')}")
    # --- End Environment Variable Settings ---

    if not os.path.exists(MODEL_PATH):
        print(f"FATAL: YOLO model not found at {MODEL_PATH}. Ensure the path is correct."); exit(1)
    ...
    # For 'spawn' start method, especially on Windows or macOS for Python 3.8+
    # if multiprocessing.get_start_method(allow_none=True) is None or multiprocessing.get_start_method() == 'fork':
    #     try:
    #         multiprocessing.set_start_method('spawn', force=True)
    #         print("INFO: Multiprocessing start method set to 'spawn'.")
    #     except RuntimeError:
    #         print("INFO: Could not set multiprocessing start method to 'spawn', using default.")
    # This is an optional enhancement for CUDA safety if 'fork' is problematic,
    # but per "do not change any other functionality", I'll leave it commented unless strictly needed.
    # Moving model init into worker is the primary fix attempt for CUDA+multiprocessing.
